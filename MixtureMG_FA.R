...
MixtureMG_FA <- function(Xsup,N_gs,nclust,nfactors,Maxiter = 1000,start = 1,nruns = 50){
  ...
  S_gs <- matrix(list(NA),nrow = ngroup, ncol = nclust)
  for(g in 1:ngroup){
...
MixtureMG_FA_Mstep <- function(S_gs,N_gs,nvar,nclust,nfactors,N_gks,Beta_gks,Theta_gks,Lambda_ks,Psi_gs,Phi_gks){
  ...
  for(g in 1:ngroup){
    ...
    for(k in 1:nclust){
      ...
      beta_gk=Beta_gks[[g,k]]
      theta_gk=Theta_gks[[g,k]]
      sum2SbetaB_BthetaB=sum2SbetaB_BthetaB+(N_gks[g,k]/N_gs[g])*(2*lambda_k%*%beta_gk%*%S_g-lambda_k%*%theta_gk%*%t(lambda_k)) # modelimplied reduced covariance matrix on sample level, based on old structure matrix and sigma_ik, weighting based on new z_gks
    }
